	...
	public Demo() {
		...
		btnConnect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if ((address.equalsIgnoreCase(address) && port.equals("8000") && address.equals("localhost"))) {
					JOptionPane.showMessageDialog(null, "Kết nối thành công!");
					txt_n.setEditable(true);
		...
		btnConnect.setBounds(188, 146, 144, 35);
		panel.add(btnConnect);

		btnDisconnect.setEnabled(false);
		btnDisconnect.setFont(new Font("Tahoma", Font.PLAIN, 15));
		btnDisconnect.setBounds(360, 146, 144, 35);
		...
		btnPS.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					for (int i = 0; i < N; i++) {
						ps[i] = new PhanSo();
						ps[i].TuSo = rand.nextInt(100) + 1;
						ps[i].MauSo = rand.nextInt(100) + 1;
...
//                  Send PS
					...
					if(data.length > 65507) {
						JOptionPane.showMessageDialog(null, "UDP datagrams can't be larger than 64K. \nPlease input another N number!");
						txt_n.requestFocus();
						return;
					}
...
//                  Receive data
					...
					DatagramPacket recData = new DatagramPacket(recByte, recByte.length, InetAddress.getByName(address),
							port);
					...
					txt_nguyenTo.setText(psArray.toString());
